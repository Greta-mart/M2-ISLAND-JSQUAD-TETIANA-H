...
public abstract class Animal extends Organism {
     ...
     * <p>
     * Клітинка виходу і клітинка входу можуть у цю ж мить оброблятися різними потоками
     * (завдання життєвого циклу ділить клітинки між робочими потоками пулу), тому перехід
     * бере лок ОБОХ клітинок одразу - інакше можлива ситуація, коли дві тварини одночасно
     * займуть "останнє" вільне місце. Щоб дві тварини, які йдуть назустріч, не заблокували
     * одна одну (класичний дедлок), лок завжди береться в одному й тому ж порядку - спершу
     * клітинка з меншим (row, col).
     */
    public void move() {
        Species me = species();
        for (int step = 0; step < me.speed(); step++) {
            ...
            Cell from = cell();
            Cell next = island().neighbour(from, direction);
            if (next == null) continue;                                  // море
            Cell first = orderKey(from) <= orderKey(next) ? from : next;
            Cell second = first == from ? next : from;
            synchronized (first) {
                synchronized (second) {
                    if (!next.hasRoom(me)) continue;                      // тісно
                    from.remove(this);
                    next.add(this);
                }
            }
    ...
    /** Порядок для послідовного блокування двох клітинок - завжди одна й та сама черга. */
    private static int orderKey(Cell cell) {
        return cell.row() * 1_000_000 + cell.col();
    }
