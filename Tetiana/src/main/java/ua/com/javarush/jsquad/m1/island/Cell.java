 ...
 * </ul>
 * <p>
 * <b>Багатопотоковість.</b> Завдання життєвого циклу тварин ({@link Island}) обробляє клітинки
 * паралельно в звичайному пулі потоків, тому одна клітинка може одночасно приймати тварину,
 * яка в неї заходить (з чужого потоку, що обробляє сусідню клітинку) і тварину, яка з неї
 * виходить (зі свого потоку). Усі методи, що читають чи змінюють {@link #residents},
 * синхронізовані на моніторі самої клітинки - {@code Animal.move()} додатково бере лок обох
 * клітинок (своєї і сусідньої) у послідовності "менший (row,col) - перший", щоб уникнути
 * дедлоку при зустрічному русі.
 */
public final class Cell {
    ...
    public synchronized int count(Species species) {
        return residents.get(species.index()).size();
    }

    public synchronized boolean hasRoom(Species species) {
    ...
    public synchronized Organism anyOf(Species species, Dice dice) {
        List<Organism> group = residents.get(species.index());
        return group.isEmpty() ? null : group.get(dice.next(group.size()));
    }

    /**
     * Складає тварин клітинки у переданий список. Буфер - свій на кожен потік
     * (кожен робочий потік завдання життєвого циклу тримає власний), тому сміття немає
     * і потоки одне одному не заважають.
     * <p>
     ...
     */
    public synchronized void collectAnimals(Ecosystem ecosystem, List<Animal> target) {
